...
use crate::solve::{assert, solve_toplevel};
    ...
    match File::open(filename) {
        ...
        Ok(mut f) => {
            ...
            match f.read_to_string(&mut s) {
                Err(e) => Status::Err(e),
                Ok(_) => match PARSER.parse(heap, &s, Lexer::new(&s)) {
    ...
    let interpreter = thread::Builder::new()
        ...
        .spawn(move || {
            ...
            match PARSER.parse(&mut heap, prelude_str, Lexer::new(prelude_str)) {
                Ok(cmds) => {
                    match exec_cmds(&mut db, &mut heap, &cmds, &mut rl, &interrupted, max_depth) {
                ...
                match PARSER.parse(&mut heap, &contents, Lexer::new(&contents)) {
                    Ok(cmds) => {
                        match exec_cmds(&mut db, &mut heap, &cmds, &mut rl, &interrupted, max_depth)
                        {
                            ...
                            Status::Err(_) => {
                                panic!("Exiting due to unexpected error in user input")
                            }
                            _ => (),
                        }
                    }
